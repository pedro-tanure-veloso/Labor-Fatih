...
program main
    ...
    integer num_points, factor, track, i, last
    ...
    do  i=1,num_points
        ...
        if (track>factor-1) then 
            ...
            grid_real_y(i+factor)=util2(grid_x(i))
            track=1
            last=i
        else 
           grid_real_y(i+factor)=util2(grid_x(i))
           grid_interpol_y(i)=linear_inter(grid_x(last),grid_real_y(last),grid_x(last+factor),grid_real_y(last+factor),grid_x(i))
           track=track+1
        end if 
